...
export default function LoginPage() {
    ...
    // Login Form
    const { register: registerLogin, handleSubmit: handleSubmitLogin, formState: { errors: errorsLogin } } = useForm<z.infer<typeof authSchema>>({
        resolver: zodResolver(authSchema),
    });

    // Signup Form
    const { register: registerSignup, handleSubmit: handleSubmitSignup, formState: { errors: errorsSignup } } = useForm<z.infer<typeof authSchema>>({
        resolver: zodResolver(authSchema),
    });
    ...
    const onSignup = async (data: z.infer<typeof authSchema>) => {
        ...
        try {
            const { error } = await supabase.auth.signUp({
                ...
                options: {
                    data: {
                        full_name: data.email.split('@')[0], // Default username
                    }
                }
            });
            if (error) throw error;
                    ...
                    <TabsContent value="signup">
                        <Card className="border-border bg-[#121212]">
                            <CardHeader>
                                ...
                                <CardDescription>Start managing your funded accounts today.</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                {error && (
                                    <div className="p-3 text-sm text-red-500 bg-red-500/10 border border-red-500/20 rounded-md">
